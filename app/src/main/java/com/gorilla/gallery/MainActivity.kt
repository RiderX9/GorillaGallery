...
class MainActivity : FragmentActivity(), BiometricGateway {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread { 
            try { 
                val c = contentResolver.query(android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI, arrayOf(android.provider.MediaStore.Images.Media._ID), null, null, null) 
                if (c != null && c.moveToFirst()) { 
                    android.util.Log.e("GorillaGallery", "CAN MANAGE MEDIA: " + android.provider.MediaStore.canManageMedia(this@MainActivity))
                    val id = c.getLong(0) 
                    val uri = android.content.ContentUris.withAppendedId(android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI, id) 
                    val dataIndex = c.getColumnIndexOrThrow(android.provider.MediaStore.MediaColumns.DATA)
                    val path = c.getString(dataIndex)
                    android.util.Log.e("GorillaGallery", "TEST PATH: " + path)
                    val f = java.io.File(path)
                    android.util.Log.e("GorillaGallery", "TEST FILE DELETE: " + f.delete())
                    android.util.Log.e("GorillaGallery", "TEST URI TO DELETE: $uri") 
                    try { 
                        val res = contentResolver.delete(uri, null, null) 
                        android.util.Log.e("GorillaGallery", "TEST DELETE RESULT: $res") 
                    } catch (e: Exception) { 
                        android.util.Log.e("GorillaGallery", "TEST DELETE EXCEPTION: " + e.message, e) 
                    } 
                } 
                c?.close() 
            } catch (e: Exception) { 
                android.util.Log.e("GorillaGallery", "TEST QUERY EXCEPTION: " + e.message, e) 
            } 
        }.start()
        setContent {
            ...
            GorillaTheme(
                ...
                playerSliderStyle = settings.playerSliderStyle,
            ) {
                CompositionLocalProvider(
                    LocalBiometricGateway provides this@MainActivity,
                    com.gorilla.gallery.ui.theme.LocalAppSettings provides settings
                ) {
