...
class VideoEditorRepository(private val context: Context) {
    suspend fun saveEditedCopy(
        ...
        filterPreset: com.gorilla.gallery.data.model.FilterPreset?,
        ...
        // Use MediaMetadataRetriever to get the REAL intrinsic dimensions, because item.width/height might be 0 if MediaStore hasn't scanned yet.
        val retriever = android.media.MediaMetadataRetriever()
        retriever.setDataSource(context, item.uri)
        val realWidth = retriever.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH)?.toIntOrNull() ?: 1920
        val realHeight = retriever.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT)?.toIntOrNull() ?: 1080
        val realRotation = retriever.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_ROTATION)?.toIntOrNull() ?: 0
        retriever.release()

        val isPortrait = (realRotation + rotationDegrees) % 180 != 0
        val bw = if (isPortrait) realHeight else realWidth
        val bh = if (isPortrait) realWidth else realHeight

        if (filterPreset != null && filterPreset != com.gorilla.gallery.data.model.FilterPreset.ORIGINAL) {
            com.gorilla.gallery.data.model.VideoEffects.createFilterLut(filterPreset)?.let { lutEffect ->
                effects.add(lutEffect)
            }
        }

        // Fix the stretch bug: Force the encoder output dimensions to match the rotated frames.
        var finalW = if (cropRect != null) (bw * cropRect.width()).toInt() else bw
        var finalH = if (cropRect != null) (bh * cropRect.height()).toInt() else bh
        finalW -= finalW % 2
        finalH -= finalH % 2
        
        effects.add(androidx.media3.effect.Presentation.createForWidthAndHeight(finalW, finalH, androidx.media3.effect.Presentation.LAYOUT_SCALE_TO_FIT))
