...
class PhotoEditorRepository(
    ...
    suspend fun render(uri: Uri, edit: EditState): Bitmap = withContext(Dispatchers.Default) {
        ...
        val rotated = if (edit.rotationDeg % 360f != 0f || edit.flipHorizontal) {
            Bitmap.createBitmap(src, 0, 0, src.width, src.height,
                Matrix().apply {
                    if (edit.flipHorizontal) postScale(-1f, 1f, src.width / 2f, src.height / 2f)
                    if (edit.rotationDeg % 360f != 0f) postRotate(edit.rotationDeg)
                }, true)
