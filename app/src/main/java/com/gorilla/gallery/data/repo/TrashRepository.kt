...
class TrashRepository(
    ...
    suspend fun directTrash(items: List<MediaItem>) = withContext(Dispatchers.IO) {
        dir.mkdirs()
        for (item in items) {
            val id = UUID.randomUUID().toString()
            val ext = MediaIo.extOf(item.mimeType, if (item.isVideo) "mp4" else "jpg")
            ...
            try {
                java.io.File(android.os.Environment.getExternalStorageDirectory(), item.relativePath + item.displayName).delete()
        ...
        mediaRepo.dropCached(items.map { it.id })
    }

    suspend fun prepareDeleteForeverOriginals(items: List<MediaItem>): TrashPrep = withContext(Dispatchers.IO) {
        TrashPrep(
            sender = mediaRepo.createDeleteRequest(items.map { it.uri }),
            mediaIds = items.map { it.id },
            trashIds = emptyList(),
    ...
    suspend fun directDeleteForeverOriginals(items: List<MediaItem>) = withContext(Dispatchers.IO) {
        for (item in items) {
            try {
                java.io.File(android.os.Environment.getExternalStorageDirectory(), item.relativePath + item.displayName).delete()
                context.contentResolver.delete(item.uri, null, null)
